...
class frame_store: #last_frame_keyPoint(x_cord,y_cord,key_sand_des)
    def __init__(self, x_cord, y_cord,keypoints, key_sand_des,xy_arrayP, Rotation, translation):
...
#for z in range(0, len(images)):
    ...
    if z == 0:  # avoid over riding last frame values
        #xy_arrayP = xy_array
        last_frame = frame_store(x_cord, y_cord, keypoints, key_sand_des, xy_array, None, None)
    ...
    Point1 = Point1[:min_size,:] #all columns
    ...
    #print('iteration z= ', z, '\n R',R,)
    # print('t=', t, '\n')
    Rotation= np.append(Rotation,R)
    translation = np.append(translation, t)
    ...
    adj_coord = np.matmul(diag, t_i)
    final_pose = adj_coord.flatten().tolist()  # tolist()= make a list

    print('mono_coordinate', mono_coordinate)
    print('len(mono_coordinate)', len(mono_coordinate))

    draw_x, draw_y, draw_z = [int(round(x)) for x in mono_coord]
    #draw_x, draw_y, draw_z = [math.floor(x) for x in mono_coord]
    ...
    #mono_coordinates.append(final_pose)
    #mono_coordinates.append(final_pose)

    #xy_arrayP = xy_array
    last_frame = frame_store(x_cord, y_cord, keypoints, key_sand_des, xy_array, R,t)

########################################################################

######### save to excel ####
# convert your array into a dataframe
df = pd.DataFrame(Rotation).T
df.to_excel(excel_writer="C:/Users/JARVIS/Documents/Tech drive/Dissertation/old experiments run/outputs/Rmatrix.xlsx")

df = pd.DataFrame(translation).T
df.to_excel(excel_writer="C:/Users/JARVIS/Documents/Tech drive/Dissertation/old experiments run/outputs/transMatrix.xlsx")
#######################
print('iteration count z =',z)
print('R',Rotation)
